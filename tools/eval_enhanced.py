...
def get_args_parser(add_help=True):
    parser = argparse.ArgumentParser(description='YOLOv6 PyTorch Evaluating with Enhanced NMS Support', add_help=add_help)
    ...
    # Enhanced NMS arguments
    parser.add_argument('--nms', type=str, default='original', choices=['original', 'enhanced'], 
                       help='NMS mode: original (standard NMS) or enhanced (Soft NMS/DIoU NMS)')
    parser.add_argument('--nms-method', type=str, default='soft', choices=['soft', 'diou', 'nms'],
                       help='Enhanced NMS method: soft (Soft-NMS), diou (DIoU-NMS), or nms (standard)')
    parser.add_argument('--nms-soft-method', type=str, default='gaussian', choices=['gaussian', 'linear'],
                       help='Soft-NMS method: gaussian or linear (only used when --nms-method=soft)')
    parser.add_argument('--nms-sigma', type=float, default=0.5,
                       help='Sigma parameter for Gaussian Soft-NMS (only used when --nms-method=soft and --nms-soft-method=gaussian)')
...
def run(data,
    ...
    """ Run the evaluation process with Enhanced NMS support
    ...
    - nms: 'original' (standard NMS) or 'enhanced' (Soft NMS/DIoU NMS)
    - nms_method: 'soft' (Soft-NMS), 'diou' (DIoU-NMS), or 'nms' (standard)
    - nms_soft_method: 'gaussian' or 'linear' (only for Soft-NMS)
    - nms_sigma: sigma parameter for Gaussian Soft-NMS

    """

     # task
    EvalerEnhanced.check_task(task)
    if task == 'train':
        save_dir = save_dir
    else:
    ...
    EvalerEnhanced.check_thres(conf_thres, iou_thres, task)
    device = EvalerEnhanced.reload_device(device, model, task)
    half = device.type != 'cpu' and half
    data = EvalerEnhanced.reload_dataset(data, task) if isinstance(data, str) else data
    ...
    val = EvalerEnhanced(data, batch_size, img_size, conf_thres, 
                ...
                nms_mode=nms, nms_method=nms_method, nms_soft_method=nms_soft_method,
