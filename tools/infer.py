...
def get_args_parser(add_help=True):
    ...
    parser.add_argument('--nms', type=str, default='original', choices=['original','enhanced'], help='NMS mode: original or enhanced')
    parser.add_argument('--nms-method', type=str, default='soft', choices=['soft','diou','nms'], help='When enhanced NMS selected, method to use')
    parser.add_argument('--nms-soft-method', type=str, default='gaussian', choices=['gaussian','linear'], help='Soft-NMS variant')
    parser.add_argument('--nms-sigma', type=float, default=0.5, help='Soft-NMS sigma for gaussian')
    parser.add_argument('--nms-topk', type=int, default=300, help='pre-NMS top-k per class to keep (smaller -> faster)')
...
def run(weights=osp.join(ROOT, 'yolov6s.pt'),
    ...
    # Inference
    # create inferer with selected nms options
    inferer = Inferer(source, webcam, webcam_addr, weights, device, yaml, img_size, half, nms_mode=nms, nms_method=nms_method, nms_soft_method=nms_soft_method, nms_sigma=nms_sigma)
